...
import { Trophy, X, Copy, Check, RefreshCw } from 'lucide-react'
import { getRole, ROLE_CHANGED_EVENT } from '../../lib/session'
import { buildFinalResultsEmail, FINAL_RESULTS_EMAIL_SUBJECT } from '../../lib/final-results-email'
import { LeaderboardEntry } from '../../lib/types'

// Admin-only: the trophy icon next to "RACE UPDATE EMAIL" — generates the
// Tour's final-results email (overall winner + Week 1/2/3 champions) from
// the same leaderboard entries already on the page. Same copy-as-rich-HTML
// flow as WelcomeEmailButton, just a different template/subject.
export default function FinalResultsEmailButton({ entries }: { entries: LeaderboardEntry[] }) {
  ...
  function generateEmail() {
    const generated = buildFinalResultsEmail(entries, window.location.origin, new Date())
    setHtml(generated)
    setCopied(false)
  }
  ...
  async function copyEmail() {
    const content = html || buildFinalResultsEmail(entries, window.location.origin, new Date())
    try {
      const item = new ClipboardItem({
        'text/html': new Blob([content], { type: 'text/html' }),
        'text/plain': new Blob(['Tour de Callisto — Final Results'], { type: 'text/plain' })
      })
  ...
  return (
    <>
      <button
        ...
        title="Generate the final results email — overall winner + weekly champions"
      >
        <Trophy size={13} />
        FINAL RESULTS EMAIL
      </button>
          ...
          <div className="w-full max-w-[1500px] h-[94vh] rounded-lg app-surface border border-border flex flex-col overflow-hidden shadow-2xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-border shrink-0">
              <div>
                ...
                <div className="text-xs text-secondaryText mt-0.5">Subject: {FINAL_RESULTS_EMAIL_SUBJECT}</div>
              </div>
            ...
            <div className="flex-1 min-h-0 flex bg-[#111517]">
              <div className="flex-1 min-w-0 overflow-auto p-4 bg-[#090D0F]">
                <div className="min-w-[1120px] flex justify-center">
                  {html && (
                    <iframe
                      title="Final results email preview"
                      srcDoc={html}
              ...
              <aside className="w-[300px] shrink-0 border-l border-border bg-[#0B1114] p-5 overflow-y-auto">
                ...
                <div className="mt-5 rounded-lg border border-border bg-[#10181C] p-4">
                  ...
                  <p className="text-xs text-secondaryText mt-2 leading-5">
                    The winner and weekly champions are computed from the same leaderboard data currently shown on this page. Generate again after the sheet refreshes to capture the latest standings.
                  </p>
                </div>
                ...
                <div className="mt-4 rounded-lg border border-border bg-[#10181C] p-4">
                  ...
                  <ol className="text-xs text-secondaryText mt-3 space-y-2 leading-5 list-decimal list-inside">
                    <li>Generate the final results email.</li>
            ...
            <div className="flex items-center justify-between px-5 py-3 border-t border-border shrink-0">
              <span className="text-[11px] text-secondaryText">Wide desktop email • overall winner + Week 1–3 champions • no automatic sending</span>
